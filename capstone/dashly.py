...
options = list(map(lambda x: {'label': x, 'value': x}, sites))
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        # site_data['class'] = site_data['class'].replace(0, 'Failure')
        # site_data['class'] = site_data['class'].replace(1, 'Success')
        fig = px.pie(site_data, values='total', 
        names='class', 
        title=title)
        return fig
...
def get_payload_plot_chart(entered_site, payload):
    filtered_df = spacex_df
    ...
    if entered_site == 'ALL':
        ...
        fig = px.scatter(site_data, y='class', x='Payload Mass (kg)', color='Booster Version Category',
        title='test')
        return fig
